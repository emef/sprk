...
echo "********  Building sprk Android native libraries"
( cd ../../../builds/android && ./build.sh )

#   Ensure we've built JNI interface
echo "********  Building sprk JNI interface & classes"
( cd .. && gradle build jar )
...
echo "********  Building sprk JNI for Android"
make $MAKE_OPTIONS

echo "********  Building sprk.jar for Android"
#   Copy class files into org/zeromq/etc.
unzip -q ../../build/libs/sprk*.jar

#   Copy native libraries into lib/armeabi
mkdir -p lib/armeabi
mv libsprkjni.so lib/armeabi
...
zip -r -m ../sprk-android.jar lib/ org/ META-INF/
cd ..
rm -rf build
